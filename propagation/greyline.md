...
description: Using the dawn/dusk terminator for enhanced HF propagation
published: true
date: 2026-04-05T09:30:00.000Z
tags: propagation, greyline, hf, dx
editor: markdown
...
The greyline is not a sharp line but a zone, roughly 200–400 km (125–250 miles) wide, where the Sun is near the horizon. Within this zone, the [ionosphere](/propagation/ionosphere) is in a transitional state: the D layer (which absorbs HF signals) is either forming (at sunrise) or decaying (at sunset), while the F layer still has enough ionization to refract signals.

## Why greyline propagation works
...
1. Determine your local sunrise and sunset times.
2. Find which locations on the other side of the world are experiencing the opposite twilight at that same moment (i.e., if you're at sunrise, find who's at sunset, and vice versa).
3. These are your most likely greyline DX targets.

### Greyline maps

Several [propagation tools](/propagation/propagation-tools) display the greyline on a world map in real time. Some of the most popular include:

- **DX Atlas:** A well-known ham radio mapping program that shows the greyline with real-time updates.
- **VOACap / HFProp:** Propagation prediction tools that can model greyline enhancement.
- **Online greyline maps:** Many websites provide free, real-time greyline displays overlaid on a world map.
- **Logging software:** Many modern logging programs (such as N1MM+, Log4OM, and others) include built-in greyline displays.
...
## Which bands benefit from greyline propagation?
...
## Operating techniques for greyline

### Timing

The greyline window is short — typically 15 to 45 minutes around sunrise and sunset. You need to be ready before it arrives:

- Check a greyline map to know which DX areas will be along the terminator at your sunrise and sunset times.
- Be on frequency and listening 15–20 minutes before local sunrise or sunset.
- The band may open suddenly and briefly. A station that was inaudible may appear with a strong signal for just a few minutes before fading again.
- Pay attention to both your sunrise and sunset — each offers different path possibilities.

### Frequency choice

On 160 m and 80 m, listen for DX stations calling CQ around the popular DX windows:
- 160 m: 1.820–1.845 MHz (CW) and the voice segments where your licence permits.
- 80 m: 3.500–3.530 MHz (CW) and 3.790–3.800 MHz (DX voice window in Region 1 and 2).
...
- Have your station ready to go. Antenna tuned, frequency selected, logging software open.
- Use the DX cluster to spot greyline openings in real time. If someone else spots a rare station appearing on 160 m at sunset, you know the greyline is active.
- Keep your calls short and efficient. In a brief opening, you may only have time for a quick exchange.
- Consider using [FT8](/digital-modes/ft8) or CW, which are more effective than SSB when signal levels are marginal and time is short.

## Greyline and long path
...
- [HF Propagation](/propagation/hf-propagation) — Comprehensive guide to HF bands and propagation
- [Band Conditions](/propagation/band-conditions) — Monitoring conditions for greyline openings
- [Propagation Tools](/propagation/propagation-tools) — Software with greyline displays
- [The Ionosphere](/propagation/ionosphere) — Understanding D-layer absorption and F-layer reflection
