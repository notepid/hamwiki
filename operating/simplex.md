...
description: Direct radio-to-radio communication without repeaters on VHF, UHF, and HF
published: true
date: 2026-04-05T09:30:00.000Z
tags: operating, simplex
editor: markdown
...
**Simplex** means communicating directly from one radio to another on the same frequency, without using a [repeater](/operating/repeaters). All HF communication is simplex (or split-frequency simplex), and on VHF/UHF it is the simplest and most fundamental form of communication — no infrastructure required, just two radios on the same frequency.
...
- **No infrastructure needed.** Simplex works anywhere — in the field, in an emergency, in areas without repeater coverage.
- **Frees up repeaters.** If you can hear another station directly, it is good practice to move to simplex so the repeater remains available for stations that need it.
- **Privacy and simplicity.** Your conversation is only heard by stations close enough to receive both of you.
- **Emergency preparedness.** When repeaters fail (power outage, equipment failure), simplex is what remains. Knowing how to operate simplex is an important skill.
- **Portable and outdoor operations.** Activities like [POTA](/awards/pota) and [SOTA](/awards/sota) often rely on simplex.
...
### Common VHF/UHF calling frequencies
...
1. **Monitor the calling frequency** and make your call (CQ or a specific station).
2. **Once contact is established**, suggest moving to a working frequency: "Let's move to 146.550."
3. **QSY** (change frequency) to the agreed-upon working frequency and continue your conversation there.
...
The exception is **split operation** on HF, where a station (often a rare DX station) transmits on one frequency but listens on a different one. This is not true duplex in the repeater sense, but it does involve two frequencies. See [DX Operating](/operating/dx-operating) for more on split operating.

## Tips for Effective Simplex Operation

- **Get your antenna as high as possible.** Even a few metres of additional height can make a significant difference.
- **Consider a better antenna.** If you are using a handheld, a roll-up J-pole antenna hung from a tree branch dramatically outperforms the stock rubber duck.
- **Use the right power level.** Start with higher power if needed, then reduce once contact is established. On FM, there is no advantage to using more power than necessary.
- **Listen before calling.** On busy simplex frequencies, wait for a clear moment before calling.
- **Give your location.** On simplex, knowing each other's location helps determine if the contact is feasible and adds useful information for signal reports.
- **Be patient with weak signals.** Simplex contacts at the edge of range can be challenging but rewarding.

## Simplex Repeaters and Parrot
...
- [Repeaters](/operating/repeaters) — Understanding repeater operation
- [VHF/UHF Operating](/operating/vhf-uhf-operating) — The bands where simplex is most used
- [Portable Operating](/operating/portable-operating) — Operating outdoors where simplex shines
- [QSO Basics](/operating/qso-basics) — How to make contacts
- [Emergency Communications](/emergency-communications/overview) — Simplex in emergency situations
