---
title: DX Operating
description: Tips and techniques for making long-distance (DX) contacts on amateur radio
published: true
date: 2026-04-05T09:30:00.000Z
tags: operating, dx, hf
editor: markdown
...
For the purposes of awards like [DXCC](/awards/dxcc), the world is divided into "entities" (roughly corresponding to countries and territories). There are currently around 340 DXCC entities, and working as many as possible is the goal of many DXers.
...
- **Web-based clusters** — Sites like DXSummit (dxsummit.fi), DX Heat, and others provide browser-based access to live spots.
- **Telnet clusters** — Direct connections to cluster nodes, often integrated into logging software.
- **Integration with logging software** — Programs like N1MM+, Logger32, Ham Radio Deluxe, and others display DX spots in real time, highlighting stations you need for awards.
- **Mobile apps** — Several apps provide DX cluster access on phones and tablets.
...
1. **Set VFO A** to the DX station's transmit frequency (where you listen).
2. **Set VFO B** to a frequency within the DX station's listening range.
3. **Enable split mode** on your radio.
4. **Listen carefully** to determine where the DX station is picking up callers. Adjust your transmit frequency accordingly.
...
## DX Awards and Accomplishments

DXing has a well-established system of awards that provide goals and recognition:

- **[DXCC](/awards/dxcc)** (DX Century Club) — The most prestigious DX award. Awarded for confirmed contacts with 100 or more DXCC entities. Higher-level endorsements (200, 300, etc.) are available up to the Honor Roll (working all but a few current entities).
- **[WAZ](/awards/waz)** (Worked All Zones) — Working all 40 CQ zones around the world.
- **[WAS](/awards/was)** (Worked All States) — Working all 50 US states (popular among both US and international operators).
- **IOTA** (Islands on the Air) — Working stations on islands around the world, organized by island groups.
- **Challenge awards** — DXCC Challenge credits contacts with unique entities on each band separately, encouraging DX work across all HF bands.

## Building a DX Station
...
## DX Ethics and Operating Practices

The DX community has developed a strong set of operating norms:

- **Do not tune up on a DX frequency.** Tune on a nearby clear frequency, then move.
- **Do not call continuously.** Transmit your callsign once, then listen. Repeat if the DX station calls again.
- **Respect the DX station's instructions.** If they ask for specific regions (e.g., "Europe only" or "Japan only"), do not call if you are not in that region.
- **Do not "police" the frequency.** Self-appointed frequency police who shout at other operators cause more disruption than the problems they are trying to solve.
- **Confirm contacts through the proper channels.** Use [QSL cards](/operating/qsl-cards) or electronic confirmation systems like LoTW or eQSL. See [QSL Cards](/operating/qsl-cards) for details.
- **Follow the DX Code of Conduct.** This widely respected set of guidelines emphasizes courtesy, patience, and fair play.

## Where to Go Next

- [DXCC](/awards/dxcc) — The premier DX award
- [HF Operating](/operating/hf-operating) — Operating on the HF bands
- [Propagation Overview](/propagation/overview) — Understanding how signals travel
- [QSL Cards](/operating/qsl-cards) — Confirming your contacts
- [Contesting Overview](/contesting/overview) — Competitive operating that sharpens DX skills
- [Antennas Overview](/antennas/overview) — Improving your signal
